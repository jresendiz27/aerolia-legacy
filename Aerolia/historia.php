...
if ($user_card->is_session_active()) {
    $result = DataLog::request_history($user_card->aero[0], $_GET["order"], $db_wrapper);
}
?>
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        ?><div id="main">
                ...
                <div id="tabs">
                    <ul>
                        <li><a href="#tabs-1">Historial de eventos</a></li>
                    </ul>
                            ...
                            <form action="historia.php" method="get">
                                <p>
                                    ...
                                    <select name="order" style="width: auto;">
                                        <option value="1">M&aacute;s reciente</option>
                                        <option value="2">Menos reciente</option>
                                        <option value="3">ID Evento - M&aacute;s reciente</option>
                                        <option value="4">ID Evento - Menos reciente</option>
                                    </select>
                                    <input type="submit" value="Ordenar" style="width: auto;"/>
                                </p>
                            </form>
                            <div id="result" style="text-align: left;">
                            <?php
                            foreach ($result as $period => $period_data) {

                                print "<table id = \"history\" style=\"border: #cdfb74 solid medium; text-align: left;\">";
                                print "<tbody>";
                                print "<tr><td colspan=\"3\">" . $period . "</th></tr>";
                                print "<tr><th>Fecha del evento</th><th>ID</th><th>Descripción</th></tr>";

                                foreach ($period_data as $info) {
                                    print "<tr><td>" . $info["fecha_historial"] . "</td><td>" . $info["id_evento"] . "</td><td>" . $info["desc_evento"] . "</td></tr>";
                                }
                                print "</tbody>";
                                print "</table>";
                            }
